...
tests = {
    "Functional": [
        # {"name": "Data Collection Verification", "command": "cat /var/log/sensorz/sensord_main.log"},
        # {"name": "Anomaly Detection Test", "command": "grep 'Anomaly Detected'
        # /var/log/sensorz/apps_garbage-detector.log"}, {"name": "Sensor Restart Recovery Test", "command": "sudo
        # reboot && tail -f /var/log/sensorz/HealthMonitoring.log"}, {"name": "Remote Reboot Capability Test",
        # "command": "ssh -t sensorz@(ip_address_sensor) 'sudo reboot'"}, {"name": "Service Restart Test",
        # "command": "ssh sensorz@(ip_address_sensor) -C 'sudo systemctl restart sensorz_service'"}, {"name": "Error
        # Log Check", "command": "ssh sensorz@(ip_address_sensor) -C 'grep 'ERROR' /var/log/sensorz.log'"},
    ],
    "Performance": [
        {"name": "CPU Load Test",
        ...
        {"name": "Network Throughput Test",
         "command": "iperf3 -c iperf.server -t 10 && cat /var/log/sensorz/sensorz_mon.log | grep 'Network Throughput'"},
        {"name": "Data Processing Rate Test", "command": "cat /var/log/sensorz/rf_scanner.log | grep 'Data Processed'"},
    ],
    "Stability": [
        {"name": "Temperature Stability Test", "command": "cat /var/log/sensorz/utils_health.log | grep 'Temperature'"},
        # {"name": "Power Fluctuation Recovery Test", "command": "./data_consistency_check --days 30"},
        # {"name": "Network Reconnection Test", "command": "sudo sensorz_firmware_update --test"},
        # {"name": "Configuration Reload Test", "command": "sudo sensorz_reload_config"},
    ],
    "Network": [
        ...
        {"name": "Network Bandwidth Control Test",
         "command": "scp 'C:/Users/Roy Avrahami/Documents/Sensorz Docs/PoE issue/file size 1GB/1GB.bin' 'sensorz@{"
                    "ip_address_sensor}:/destination/path/on/remote/machine'"},
        {"name": "Network Packet Loss Test",
         "command": "ssh sensorz@(ip_address_sensor) -C 'mtr -rw 8.8.8.8'"},
    ],
    "Hardware": [
        {"name": "CPU Status Test",
         "command": "lscpu && cat /var/log/sensorz/HealthMonitoring.log | grep 'CPU Status'"},
        {"name": "Memory Test",
         "command": "memtester 128M 1 && grep 'Memory Test' /var/log/sensorz/HealthMonitoring.log.1"},
        {"name": "Network Interface Test",
         "command": "ssh sensorz@(ip_address_sensor) -C 'export PATH=/PATH:/sbin && ethtool eth0'"},
        {"name": "Temperature Test",
         "command": "ssh sensorz@(ip_address_sensor) -C 'sensors'"},
        {"name": "Power Supply Status Test",
         "command": "(Hardware Management Interface) Power supply status is healthy."},
        {"name": "Visual Inspection",
         "command": "Visual inspection"},
        {"name": "Critical Hardware Errors Test",
         "command": "ssh sensorz@(ip_address_sensor) -C 'dmesg | grep -i ""error'"},
        {"name": "USB Port Test",
         "command": "ssh sensorz@(ip_address_sensor) -C 'lsusb'"},
    ],
}
...
def main():
    ...
    report = generate_html_report(test_results, title)  # Pass the title here
